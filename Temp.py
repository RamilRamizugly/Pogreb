...
def on_chat_message(msg):
    ...
    if content_type == 'text':
        ...
        if command == '/start' or command == '/start@LeechDacha_bot':
            ...
            markup = ReplyKeyboardMarkup(keyboard=[
                [dict(text='Вентиляция АВТО'), dict(text='Вентиляция ВКЛ'), dict(text='Вентиляция ВЫКЛ')],
                [dict(text='Подогрев АВТО'), dict(text='Подогрев ВКЛ'), dict(text='Подогрев ВЫКЛ')],
                [dict(text='Главное меню')],
            ], resize_keyboard=True)
            bot.sendMessage(chat_id, 'Выбери объект', reply_markup=markup)

        elif command == u'Подогрев АВТО':
            a_pomp_1 = 1
            bot.sendMessage(chat_id, 'Включен авто режим для подогрева ')

        elif command == u'Вентиляция АВТО':
            a_pomp_2 = 1
            bot.sendMessage(chat_id, 'Включен авто режим для вентиляции ')

        elif command == u'Подогрев ВКЛ':
            a_pomp_1 = 0
            gpio.output(pomp_1, 0)
            bot.sendMessage(chat_id, 'Включен Подогрев ')

        elif command == u'Вентиляция ВКЛ':
            a_pomp_2 = 0
            gpio.output(pomp_2, 0)
            bot.sendMessage(chat_id, 'Включена вентиляция ')

        elif command == u'Подогрев ВЫКЛ':
            a_pomp_1 = 0
            gpio.output(pomp_1, 1)
            bot.sendMessage(chat_id, 'Выключен Подогрев ')

        elif command == u'Вентиляция ВЫКЛ':
            a_pomp_2 = 0
            gpio.output(pomp_2, 1)
            bot.sendMessage(chat_id, 'Выключена вентиляция ')
...
def bot_temp(chat_id):
    bot.sendMessage(chat_id, "Температура 1: %s\n" \
                             "Температура 2: %s\n" \
                             "Температура 3: %s\n" % (
